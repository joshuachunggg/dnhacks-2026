...
- Added a durable live-assistant collection path: explicit model tap modes persist confirmed panel/EVSE locations and route waypoints before Realtime receives tool output; the server derives feet-based route length from those waypoints. Placement mode now hides and disables the transcript/control overlay so model taps reach the SceneKit canvas, then displays a conceptual, dimensioned panel enclosure, EVSE enclosure, or route conduit at the recorded coordinates.
- Removed the manual panel-fact form. The Realtime guide now requests one missing value conversationally, asks whether it is known or a planning approximation, and records the resulting typed fact with that provenance. Realtime playback remains at its original 1× direct PCM path after reverting the incompatible time-pitch graph. Simulator and `iphoneos` builds pass; physical-device interaction remains unverified.
- RoomPlan starts automatically only after its capture view is visible and while the app is active. Temporary view disappearance and app interruption pause rather than terminally invalidate the scan, allowing a visible capture controller to restart cleanly. A completed one-room scan pauses its underlying AR session before the assistant can open its separate camera session, avoiding retained AR/camera ownership between the two capture modes. It exposes one Finish room scan control, rather than a separate start/retry flow. The delayed-start physical-device case remains to be exercised.
- Retained a saved RoomPlan model when an operator starts an address-based blank assessment. The first server-backed assessment action re-uploads the device-local USDZ and records its capture manifest on the new assessment before panel, location, or route evidence can use it.
- Removed the yellow preliminary guide marker. Compact N/E/S/W labels now sit on the captured model's bounding-wall planes as shared model-relative references; the UI explicitly marks them as non-compass directions because RoomPlan does not supply a true-north transform. The operator still chooses each conceptual panel, EVSE, and route location directly.
- Added a second Realtime demo focus for adjacent-room expansion. The guide asks bounded questions about use, approvals, visible utilities/openings, and structural review, then can request a user-selected divider-wall location. The live SceneKit view renders that location as a conceptual opening and Results repeats that it is not a structural finding, demolition instruction, permit approval, or feasibility conclusion. Simulator compilation and session-policy tests pass; physical-device Realtime/placement acceptance remains unverified.
- Added a **Load demo data** control at the bottom of each native page. It disconnects Realtime, clears any live spatial visuals, and loads the bundled modern assessment into the existing page UI with the fixture fallback label; it does not merge seeded facts with the active live assessment.

## Current work
- Validate the server-owned Realtime interview on the installed physical-device build. Typed tool calls are held until streamed audio and device playback complete, then their native camera, placement, or panel-fact control appears. Each deterministic gate transition now issues one exact visible action and tool call; completing a multi-waypoint route returns to the server-owned gate before any new guide response. The Results cost card now leads with a planning total and disclaimer, exposes each line item's range and primary factor, and keeps assumptions, warnings, source, and quantities collapsed. Physical-device acceptance remains required for the conversation and route sequence.
...
- Validate the adjacent-room expansion conversation and conceptual opening selection on a physical device, including the two-room capture boundary and Results handoff wording.
