...
struct RoomPlanCaptureCheck {
    static func main() throws {
        ...
        precondition(previewSource.contains("styleImportedGeometry(in: scene)"), "Imported RoomPlan geometry must receive an inspection-specific material treatment after loading.")
        precondition(previewSource.contains("if name.contains(\"wall\")"), "Room walls must use a distinct material color rather than merging with captured objects.")
        precondition(previewSource.contains("if name.contains(\"floor\")"), "Room floors must use a distinct material color rather than merging with walls and captured objects.")
