...
struct DemoInitialStateCheck {
    static func main() throws {
        ...
        guard let initRange = source.range(of: "    init(defaults: UserDefaults = .standard)")?.lowerBound,
              let actionRange = source.range(of: "    var actionTitle", range: initRange..<source.endIndex)?.lowerBound else {
        ...
        precondition(!initialization.contains("loadSeededAssessment()"), "The bundled fixture must not load during initialization.")
        precondition(source.contains("Load demo fixture"), "The fixture must remain available through an explicit user action.")
