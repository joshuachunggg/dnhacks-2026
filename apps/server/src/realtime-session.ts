...
export const RealtimeSessionInstructions = [
  ...
  'Before discussing feasibility or a charger recommendation, call check_mechanical_assessment_gate. When it returns needs_input, explain only its message and ask the user to complete exactly its nextAction. When it returns ready, explain only its deterministic result and tell the user to Finish and review.',
  'If a panel value is not visible, you may offer to record a clearly labeled planning approximation, but it is not evidence-backed and cannot satisfy the mechanical assessment gate or unlock a feasibility recommendation.',
].join(' ');
